...
export const createOrder = async (req, reply) => {
    try {
        ...
        const {items, branch, price} = req.body;

        const customerData = await Customer.findOne(userId);
        const branchData = await Branch.findOne(branch);
        ...
        console.error(error);
        reply.status(500).send({ error: 'An error occurred while creating the order.' });
    }
}
...
export const confirmOrder = async (req, reply) => {
    try {
        ...
        return reply.status(500).send({ message: 'Failed to confirm the order', error });
    }
}
